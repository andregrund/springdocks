...
public class SequiturImport {
    ...
    public static class FSA {
        ...
        public void afterUnmarshal(Unmarshaller unmarshaller, Object parent) {
            // might also work with other formats, but we have never seen any other format
            assert "tropical".equals(semiring);
            // add a new initial state that transduces <s>:<s> and transitions to the first real state
            ...
            // sort the states (to ascertain that initialState is the first element)
        ...
        public Fst toFst() {
            ...
            openFstStates = new ArrayList<edu.cmu.sphinx.fst.State>(states.size());
            ...
            // second pass (now that all openFst states are created) to add all the openFst arcs
            for (State state : states) {
    ...
    public static class State {
        ...
        public void afterUnmarshal(Unmarshaller unmarshaller, Object parent) {
            id++; // increment state ID (because we add a new initial state 0 transitioning via <s>:<s>)
        }

        /**
         * return a first approximation State which does not yet incoroporate arcs
         */
    ...
    public static class Arc {
        ...
        @XmlElement
        int in; // automatically set to 0 (which corresponds to epsilon) if not set in XML

        @XmlElement
        int out; // automatically set to 0 (which corresponds to epsilon) if not set in XML

        @XmlElement
        float weight;

        public void afterUnmarshal(Unmarshaller unmarshaller, Object parent) {
            target++; // increment state ID (because we add a new initial state 0 transitioning via <s>:<s>)
        }

        public edu.cmu.sphinx.fst.Arc toOpenFstArc(List<edu.cmu.sphinx.fst.State> openFstStates) {
            return new edu.cmu.sphinx.fst.Arc(in, out, weight, openFstStates.get(target));
        }
    }
    ...
    public static void main(String... args) throws JAXBException, IOException {
        ...
        System.out.println("The Sequitur G2P XML-formatted FST " + args[0]
            + " has been converted to Sphinx' OpenFst binary format in the file " + args[1]);
        // uncomment this to test your model:
        //		edu.cmu.sphinx.linguist.g2p.G2PConverter d = new edu.cmu.sphinx.linguist.g2p.G2PConverter(args[1]);
        //		List<edu.cmu.sphinx.linguist.g2p.Path> path = d.phoneticize("wahnsinn", 5);
        //		System.err.println(path);
    }

    public static void importSequitur(String input, String output) throws JAXBException, IOException {
        JAXBContext context = JAXBContext.newInstance(FSA.class);
        Unmarshaller unmarshaller = context.createUnmarshaller();
        FSA fsa = (FSA) unmarshaller.unmarshal(new File(input));
        edu.cmu.sphinx.fst.Fst fst = fsa.toFst();
        fst.saveModel(output);
        System.out.println("The Sequitur G2P XML-formatted FST " + input
            + " has been converted to Sphinx' OpenFst binary format in the file " + output);
    }
}
