...
import PostProcessor.SphinxBased.LexTreeLinguist.LexTreeUnitState;
...
public class TokenGraphDumper {
    ...
    private String dumpTokenGDL(Token token) {
        ...
        if (dumpedTokens.contains(token)) {
            ...
            else if(token.getSearchState() instanceof LexTreeUnitState)
            	label = ((LexTreeUnitState)token.getSearchState()).getUnit().getName()+ label;
