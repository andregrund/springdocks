 ...
 * <code> getInitialSearchState</code>. This method returns a SearchState.
 * Successor states can be retrieved via calls to
 * <code>SearchState.getSuccessors().</code>. There are a number of search state
 * sub-interfaces that are used to indicate different types of states in the
 * search space:
 * <p/>
 * <ul>
 ...
 * method <code>getSearchStateOrder</code> can be used to retrieve the order of
 * state returned by the linguist.
 * <p/>
 * <p/>
 ...
 * will generate the search states dynamically, that is, the object representing
 * a particular state in the search space is not created until it is needed by
 * the SearchManager. SearchManagers often need to be able to determine if a
 * particular state has been entered before by comparing states. Because
 * SearchStates may be generated dynamically, the
 * <code>SearchState.equals()</code> call (as opposed to the reference equals
 * '==' method) should be used to determine if states are equal. The states
 * returned by the linguist will generally provide very efficient
 * implementations of <code>equals</code> and <code>hashCode</code>. This will
 * allow a SearchManager to maintain collections of states in HashMaps
 * efficiently.
 * <p/>
 * <p/>
 * <p/>
 ...
 * <ul>
 * <li><b>Dynamic </b>- the linguist generates search states on the fly, greatly
 ...
 * the search.
 * <li><b>HMM sharing </b>- because of state tying in the acoustic models, it is
 * often the case that triphone units that differ in the right context actually
 * are represented by the same HMM. This linguist recognizes this case and will
 * use a single state to represent the HMM instead of two states. This can
 * greatly reduce the number of states generated by the linguist.
 ...
 * particularly helpful is to share the end word units (where the largest fanout
 * of states occurs) across all of the words. For a 60K word vocabulary, these
 * can result in a reduction in tree nodes of about 2 million to around 3,000.
 ...
 * linguist works but are not necessary if you are only interested in using this
 * linguist.
 * <p/>
 * <p/>
 * <p/>
 ...
 * initial state of each possible word. That is 60K transitions. In a tree based
 * system we need to only provide transitions to each initial phone (within its
 * context). That is about 1600 transitions. This is a substantial reduction.
 * Conceptually, this tree consists of a node for each possible initial unit.
 * Each node can have an arbitrary number of children which can be either unit
 * nodes or word nodes.
 * <p/>
 * <p/>
 * This linguist uses the HMMTree class to build and represent the tree. The
 * HMMTree is given the dictionary and language model and builds the lextree.
 * Instead of representing the nodes in the tree as phonemes and words as is
 * typically done, the HMMTree represents the tree as HMMs and words. The HMM is
 * essentially a unit within its context. This is typically a triphone (although
 * for some units (such as SIL) it is a simple phone. Representing the nodes as
 ...
 * actual number of states that are considered during a search. Experiments have
 * shown that this can reduce the required beam by a factor of 2 or 3.
 * <li>By representing the nodes as HMM, we avoid having to lookup the HMM for a
 * particular triphone during the search. This is a modest savings.
 * </ul>
 * <p/>
 ...
 * <li><b>speed </b> building the much larger HMM tree can take much more time,
 * since many more nodes are needed to represent the tree.
 * <li><b>complexity </b> representing the tree with HMMs is more complex. There
 * are multiple entry points for each word/unit that have to be dealt with.
 * </ul>
 * <p/>
 ...
 * ending nodes. There is a word ending node for each possible right context. To
 * reduce space, all of the word ending nodes are replaced by a single EndNode.
 * During the search, the actual HMM nodes for a particular EndNode are
 * generated on request. These sets of HMM nodes can be shared among different
 * word endings, and therefore are cached. The effect of using this EndNode
 * optimization is to reduce the space required by the tree by about 300mb and
 * the time required to generate the tree from about 60 seconds to about 6
 * seconds.
 *
 * <p/>
 * <b>Word Histories </b>
 * <p/>
 * We use explicit backoff for word histories. That technique is proven to be
 * useful and save number of states. The reasoning is the following. With a
 * vocabulary of size N, you have N^2 unique bigram histories. So the token
 * stack will have N^2*K unique tokens, where K is the number of states per
 * token. For a 100k vocab, 3 states per HMM, that will be 3*10^10 tokens (max).
 * Of course, a large majority of them will be pruned, but really, its still way
 * too much. If you stick with the <b>actual</b> K-gram used (i.e. accounting
 * explicitly for backoff), then this reduces <b>tremendously</b>. Most bigrams
 * dont have corresponding trigrams. Not all 10^10 bigrams have trigrams. We
 * only need to store as many explicit tokens as the number of bigrams that have
 * trigrams.
 */
public class LexTreeLinguist implements Linguist
{
     ...
     * {@link LexTreeLinguist#PROP_WANT_UNIGRAM_SMEAR}
     */
    ...
    abstract class LexTreeState implements SearchState, SearchStateArc
    {
        ...
        protected SearchStateArc[] getSuccessors(Node theNode)
        {
            ...
            if (theNode instanceof UnitNode)
            {
                arcs[i] = createBeginningUnitStateArc((UnitNode) theNode, this);
            }
            return arcs;
        }
                ...
                UnitNode lastUnit, LexTreeState previous)
        {
            ...
            float probability = languageModel.getProbability(nextWordSequence)
                    * languageWeight;
            //            System.out.println(probability+"="+languageModel.getProbability(nextWordSequence)+"*"+languageWeight);
         ...
         */
        SearchStateArc createUnitStateArc(UnitNode hmmNode,
        ...
        private SearchStateArc createBeginningUnitStateArc(UnitNode hmmNode,
                LexTreeState previous)
        {
            // TODO Auto-generated method stub
            SearchStateArc arc;
            // System.out.println("CUSA " + hmmNode);
            float insertionProbability = calculateBeginningUnitInsertionProbability(hmmNode);
            float smearProbability = getUnigramSmear(hmmNode)
                    + previous.getSmearTerm();
            float languageProbability = smearProbability
                    - previous.getSmearProb();
...
//                arcs = new SearchStateArc[nodes.length];
                ...
                arcs[nodes.length] = new LexTreeEndUnitState(this.getEndNode(),
            ...
            UnitSearchState, ScoreProvider
    {
        ...
        public float getScore(Data data)
        {
            ...
            return ((PhoneData) data).getConfusionScore(
                    getUnitNode().getBaseUnit()
                        .getName(), numberOfTimesUsed) * 1f;
            ...
            WordSearchState
    {
                ...
                float languageProbability)
        {
            ...
            //            if (wordNode.getPronunciation().getUnits().length>1)
        ...
        public SearchStateArc[] getSuccessors()
        {
            ...
            if (arcs == null)
            {
                ...
                //            Hier wird für den Frame 0 NUR eine arc zum SIL unitstate erstellt
                if (wordNode.getWord()
                    .isSentenceStartWord())
                {
                ...
                else if (wordNode.getWord() != sentenceEndWord)
                {
                    ...
                    for (Node node : list)
                    {
                        arcs[index++] = createUnitStateArc((UnitNode) node,
                                this);
                    }
     ...
     */
    private float calculateInsertionProbability(
            AbstractUnitNode abstractUnitNode)
    {
    ...
    private float calculateBeginningUnitInsertionProbability(
            AbstractUnitNode abstractUnitNode)
    {
        int type = abstractUnitNode.getType();

        if (type == AbstractUnitNode.SILENCE_UNIT)
        {
            return logSilenceInsertionProbability;
        }
        else
