...
import de.unihamburg.informatik.wtm.docks.postprocessor.sphinxbased.OurDynamicFlatLinguist.GrammarState;
import de.unihamburg.informatik.wtm.docks.postprocessor.sphinxbased.OurDynamicFlatLinguist.OurFullHMMSearchState;
import de.unihamburg.informatik.wtm.docks.postprocessor.sphinxbased.OurDynamicFlatLinguist.PronunciationState;
...
public class TokenGraphDumper {
    ...
    private String dumpTokenGDL(Token token) {
        ...
        if (dumpedTokens.contains(token)) {
            ...
            else if(token.getSearchState() instanceof OurFullHMMSearchState)
            	label = ((OurFullHMMSearchState)token.getSearchState()).getUnit().getName()+ label;
            
            //System.out.println(token+"   "+token.getSearchState().getClass().getName());
            //String label = "";
            //if(token.getSearchState() instanceof GrammarState)
            //	label = "["+((GrammarState)token.getSearchState()).getGrammarNode().getWord()+"]";

            //else if(token.getSearchState() instanceof PronunciationState)
            //	label = "["+((PronunciationState)token.getSearchState()).getPronunciation().toDetailedString()+"]";            
